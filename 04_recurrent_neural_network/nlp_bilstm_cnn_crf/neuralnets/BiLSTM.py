...
class BiLSTM:
    ...
    def __init__(self, params=None):
        ...
        defaultParams = {
            'dropout': (0.5, 0.5),          # 第1个output dropout,第2个recurrent dropout
            'classifier': ['Softmax'],      # 分类器,Softmax/CRF
            'LSTM-Size': (100,),            # LSTM size
            'customClassifier': {},         # multi-task learning scenarios
            'optimizer': 'adam',            # 优化器,SGD/AdaGrad/AdaDelta/RMSProp/Adam/Nadam
            'charEmbeddings': None,         # Theano backend scenarios
            'charEmbeddingsSize': 30,
            'charFilterSize': 30,
            'charFilterLength': 3,
            'charLSTMSize': 25,
            'maxCharLength': 25,
            'useTaskIdentifier': False,     # multi-task learning scenarios
            'clipvalue': 0,
            'clipnorm': 1,
            'earlyStopping': 5,
            'miniBatchSize': 32,
            'featureNames': ['tokens', 'casing'],   # which features the network should use
        ...
        self.learning_rate_updates = None
        self.trainSentenceLengthRanges = None

    # 初始化mapping/embedding
    def setMappings(self, mapping, embedding):
    ...
    def buildModel(self):
        ...
        for featureName in self.params['featureNames']:
            ...
            feature_embedding = Embedding(input_dim=len(self.mappings[featureName]),
                                          ...
                                          name=featureName+'_embeddings')(feature_input)
        ...
        if self.params['charEmbeddings'] not in [None, "None", "none", False, "False", "false"]:
            ...
            chars = TimeDistributed(Embedding(input_dim=charEmbeddings.shape[0], output_dim=charEmbeddings.shape[1],
            ...
            if isinstance(self.params['dropout'], (list, tuple)):  
                ...
                if self.params['dropout'] > 0.0:
                    shared_layer = TimeDistributed(
                        Dropout(self.params['dropout']), name='shared_dropout_'+str(self.params['dropout'])+"_"+str(cnt)
                    )(shared_layer)
        ...
        for modelName in self.modelNames:
            ...
            modelClassifier = self.params['customClassifier'][modelName] \
                if modelName in self.params['customClassifier'] else self.params['classifier']
            ...
            optimizerParams = {}
            if 'clipnorm' in self.params:
                if self.params['clipnorm'] is not None:
                    if self.params['clipnorm'] > 0:
                        optimizerParams['clipnorm'] = self.params['clipnorm']
            
            if 'clipvalue' in self.params:
                if self.params['clipvalue'] is not None:
